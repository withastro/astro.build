...
Currently, Astro reverses the order of style and script tags in the generated HTML output, which can give unexpected results. Changing this to preserve the order is a breaking change, so it's behind an experimental flag in Astro 5.5. 
...
As an example, consider these styles:
...
By default, Astro will generate the following output:
...
This is likely to become the default behavior in Astro 6, so it's a good idea to test your site with this flag enabled to ensure it works as expected.
