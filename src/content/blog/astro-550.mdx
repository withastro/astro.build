---
title: 'Astro 5.5'
description: 'Astro 5.5 dives deep with easier Mermaid diagram support, improved Markdown compatibility, and type-safe sessions!'
homepageLink:
  title: 'Astro 5.5'
...
**Astro 5.5 dives deep with easier Mermaid diagram support, improved Markdown compatibility, and type-safe sessions!**

🧜‍♀️ Dive into a sea of new features with these improvements to Astro:

- [**Easier Mermaid diagram support in Markdown**](#easier-mermaid-diagram-support-in-markdown)
...
## Easier Mermaid diagram support in Markdown

Mermaid is a popular diagramming and charting tool that uses text definitions inside code blocks to render charts directly inside your Markdown content. Until now, using Mermaid diagrams in Astro could be tricky because syntax highlighting would interfere with how the code blocks are rendered. Astro 5.5 introduces a new `excludeLangs` option in your Markdown syntax highlighting configuration that makes it possible to skip syntax highlighting for specific language code blocks. This unlocks the ability to use Mermaid without disabling syntax highlighting for other blocks.
...
Which will render as a diagram in your page.
...
For more information, see the [experimental session docs](https://docs.astro.build/en/reference/experimental-flags/sessions/#session-data-types).

## Improved heading ID compatibility

Astro 5.5 introduces a new `experimental.headingIdCompat` flag that improves compatibility with how heading IDs are generated in other Markdown processors like those used by GitHub and npm.
...
With the new configuration flag, you can now ensure your heading IDs follow the same conventions as other platforms:
