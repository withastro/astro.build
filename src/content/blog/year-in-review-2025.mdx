...
homepageLink:
  title: '2025'
  subtitle: 'Read the year in review!'
authors:
  - shinya
  - sarah
...
<BlogContentImage src={npmTrends} alt="A graph with jagged but consistent growth over the last 5 years, more exponential than linear." />
...
Content collections, introduced in Astro [v2.0](/blog/introducing-content-collections/), have been a powerful feature that allows you to manage and use local content such as Markdown and MDX files in a type safe manner.

In [v5.0](/blog/astro-5/), this concept was reimagined with the introduction of the Content Layer API, enabling content fetched from external CMSs and APIs to be handled with the same level of type safety as local Markdown and MDX files. However, because this content is fetched at build time, supporting highly dynamic or personalized content was difficult.

With the experimental introduction of live content collections in [v5.10.0](/blog/astro-5100/#experimental-live-content-collections), it is now possible to fetch data at runtime instead of build time via content loaders. This allows sites to access the latest data in a type safe way without requiring a rebuild whenever content is updated.
...
To address these challenges flexibly, in [v5.10.0](/blog/astro-5100/#responsive-images-are-now-stable), Astro introduced a new behavior that makes responsive images much easier to implement.

To automatically add global styles for responsive images, set [`image.responsiveStyles`](https://docs.astro.build/en/reference/configuration-reference/#imageresponsivestyles) to `true` in your Astro config file.
...
```
 ...
 This allows you to specify [a `layout` value](https://docs.astro.build/en/reference/modules/astro-assets/#layout) on the `<Image />` component to determine how the image should resize when its container changes size.
...
It is also worth noting that Astro [v5.7.0](/blog/astro-570/#svg-components) introduced support for importing SVG files as Astro components. This is another handy feature, so be sure to give it a try!
...
Astro previously did not provide a built-in approach for working with fonts, but [v5.7.0](/blog/astro-570/#experimental-fonts-api) introduced an experimental Fonts API. With this API, Astro can handle web font optimizations such as preload links, optimized fallbacks, and opinionated defaults.
...
[The official MCP inspector](https://modelcontextprotocol.io/docs/tools/inspector) is a convenient option to quickly try out the Astro Docs MCP server. You can launch it with the command `npx @modelcontextprotocol/inspector`, then choose **Streamable HTTP** as the Transport Type, and enter `https://mcp.docs.astro.build/mcp` as the URL.
...
<BlogContentImage src={mcpInspector} alt="A screenshot of the MCP Inspector connected to the Astro Docs MCP Server, showing the 'search_astro_docs' tool and documentation data for 'Astro content collections'." />

[Check out the official docs for more information on the Astro Docs MCP Server](https://docs.astro.build/en/guides/build-with-ai/#astro-docs-mcp-server).

### `astro:config`

In [v5.7.0](/blog/astro-570/#config-imports), Astro added support for importing a subset of the Astro configuration in a type safe way. The `astro:config` virtual module provides two submodules, `/client` and `/server`, which are designed to be used depending on the runtime environment.
...
- [Trailing slash redirects](/blog/astro-520/#trailing-slash-redirects)
- [Faster page rendering](/blog/astro-530/#faster-page-rendering)
- [Remote image optimization in Markdown](/blog/astro-540/#remote-image-optimization-in-markdown)
- [Experimental: preserve order of style and script tags](/blog/astro-550/#experimental-preserve-order-of-style-and-script-tags)
- [Prerendered route collision warnings](/blog/astro-5140/#prerendered-route-collision-warnings)
- [SVG optimization with SVGO](/blog/astro-5160/#svg-optimization-with-svgo)

## Starlight

[Starlight](https://starlight.astro.build/) is the official theme for building documentation sites with Astro. Since it was released in 2023, it has been steadily gaining popularity, and in 2025 Starlight powered documentation sites for [Cloudflare](https://developers.cloudflare.com/), [Google](https://geminicli.com/), [Microsoft](https://aspire.dev/), [Netlify](https://docs.netlify.com/), [OpenAI](https://openai.github.io/openai-agents-js/), [WPEngine](https://developers.wpengine.com/docs/), and countless others.

Starlight itself also continued to evolve throughout 2025, seeing seven minor releases with updates including a powerful route data middleware system for implementing custom logic, multi-site search support, automatic heading anchor links, CSS cascade layers, and several accessibility improvements, among many others. Starlight ends 2025 with:
...
On October 9 and 10, 2025, [ViteConf](https://viteconf.amsterdam/) took place in Amsterdam, the Netherlands. <Mention name="sarah" /> and <Mention name="matt" /> from the Astro team spoke about making meaningful contributions to open source projects and shared updates on where Astro is today and what is coming next. If you have not watched the talks yet, definitely check them out:
...
By the way, below is a photo taken by Evan You, the creator of Vue and Vite, showing <Mention name="fred" /> and <Mention name="matt" /> from the Astro team in conversation with Tanner Linsley from TanStack. ViteConf is truly an exciting conference where developers from many different projects come together like this!
...
The official Astro merch shop has been reintroduced as [Astro Mart](https://shop.astro.build/). Both the designs and product lineup have been refreshed, and international shipping is now available. You can also purchase items created by the winners of [the Spirit of Astro Design Contest](/blog/spirit-of-astro-winners/). Be sure to check it out and grab some Astro merch.

### Astro Agency Partner Program

In April 2025, Astro launched [the Astro Agency Partner Program](/blog/astro-agency-partners/), designed to connect businesses seeking professional development services with agencies that specialize in Astro. The program includes an agency directory, lead generation support, official certification with partner badges, direct access to resources from the Astro team, and a financial model that supports continued investment in the open source project.

Launch partners included [Bejamas](/agencies/bejamas/), [Lucky Media](/agencies/lucky-media/), [EXDST](/agencies/exdst/), and [58agents](/agencies/58agents/) with [Bits&Letters](/agencies/bits-and-letters/) and [Seibert Group](/agencies/seibert/) joining shortly after. If you are looking for expert help building your next Astro project, be sure to check out the [agency partner directory](/agencies/).

### Official Sponsors

2025 was a landmark year for Astro's sustainability, with several companies stepping up as official sponsors.

In September, [Mux](/blog/mux-official-video-partner/) became the official video partner with a $5,000 monthly sponsorship and released two Astro integrations for video playback and uploading. [Webflow](/blog/webflow-official-partner/) announced a $150,000 donation, with their AI code generation tool built entirely on Astro. [Cloudflare](/blog/cloudflare-official-partner/) also contributed $150,000 and became an official partner sponsor. Later in November, [Stainless](/blog/stainless-astro-launch/) joined as a sponsor partner, launching their Stainless Docs Platform built on Starlight.
...
Many websites were built with Astro in 2025, and the sites below are some of my personal favorites. If you find an Astro site you like, go ahead and post about it on social media or in our Discord community `#showcase` channel!

If you have built your own Astro project, you can also [submit it to the official Astro showcase](https://github.com/withastro/roadmap/discussions/521) and see your work alongside other [showcased sites](https://astro.build/showcase/).
...
Route caching is an API that aims to provide declarative cache control based on web standards in a platform independent way. The early idea behind this new feature was first teased by <Mention name="matt" /> in [his ViteConf 2025 talk](#viteconf).

Below is a sample taken from [the RFC](https://github.com/withastro/roadmap/pull/1245), which shows that you can configure values such as `maxAge`, `swr`, and `tags` on a per route basis, and invalidate caches by `path` or `tag`. It is also designed to support cache invalidation in combination with the `cacheHints` returned from `getLiveEntry()` in the Live Content Collections mentioned earlier:
