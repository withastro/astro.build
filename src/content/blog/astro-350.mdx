...
You can enable prefetching for your site with the `prefetch: true` config. It is enabled by default when using [View Transitions](https://docs.astro.build/en/guides/view-transitions/) and can also be used to configure the `prefetch` behavior used by View Transitions.
...
- You can define if prefetch occurs on `tap` (a touch that's not a full click), `hover`, or `viewport` (when the element is in view).
...
You can install the Qwik integration by running:

```shell
npx astro add @qwikdev/astro
```

Or you can install separately and update your config:
