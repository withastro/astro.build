...
[Markdoc](https://markdoc.dev/) support in Astro continues to improve with the latest release of the `@astrojs/markdoc` [integration](https://docs.astro.build/en/guides/integrations-guide/markdoc/). This new release brings full feature parity with Markdown and MDX in Astro, including automatic `id` generation in headings, better syntax highlighting support, and support for config extensions with `extends`.
