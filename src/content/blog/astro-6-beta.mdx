...
pnpm install astro@next
yarn add astro@next
```

Check out [our v6 upgrade guide](https://docs.astro.build/en/guides/upgrade-to/v6/) for full details and migration guidance for breaking changes.
...
[workerd](https://github.com/cloudflare/workerd) is Cloudflare's open-source JavaScript and WebAssembly runtime. Built on V8 (the same engine Chromium and Node.js use), workerd is:

- **Standards-compliant** - Uses web platform APIs, making code portable across runtimes
- **Production-identical** - The exact runtime powering Cloudflare Workers
- **Event-driven** - Optimized for serverless, request-driven architectures
- **Lightweight** - Designed to be portable and embeddable
...
Live content collections—introduced experimentally in Astro 5.10—are now stable in Astro 6. These allow you to fetch content at runtime from CMSs, APIs, databases, and other sources, with a unified API that works across all your content.

Unlike build-time collections, live collections update without requiring a rebuild:
...
Access live data with error handling:
...
Content Security Policy (CSP) support, previously experimental, is now stable in Astro 6. CSP helps protect your site against cross-site scripting (XSS) and other code injection attacks by controlling which resources can be loaded.
...
- **Node version** - Requires Node 21+ (dropped Node 18 & 20 support)
...
The workerd dev support is particularly important to test broadly—we want to ensure it works smoothly across different project types and configurations.
...
For Cloudflare projects, check out the [Cloudflare adapter documentation](https://docs.astro.build/en/guides/integrations-guide/cloudflare/) for setup instructions.
