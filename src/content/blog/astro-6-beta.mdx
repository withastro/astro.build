---
title: "Astro 6 Beta"
description: "Astro 6 Beta brings workerd support for Cloudflare development, an upgrade to Zod 4, and stabilizes experimental features including live collections and Content Security Policy."
...
**The new Astro 6 development server unlocks first-class support for Astro on Cloudflare Workers and other non-Node.js JavaScript runtimes,** where previously our support had been more limited to Node.js. Now, You can develop your Astro project using the same runtime as production (ex: workerd for Cloudflare), with more access to runtime-specific primitives and a more true-to-life development experience.
...
In previous versions, Astro's dev server ran on Node.js, and would attempt to *simulate* any non-Node.js deployment environments as closely as possible. This was workable for most users, but it relied on brittle polyfills, mocks, and framework-specific shims to reproduce correct production behavior.

It didn't always work. Code that worked locally could behave differently once deployed. Platform-specific features often couldn't be exercised until after deployment. In some cases, Astro even had separate logic paths for “dev” and “prod,” increasing the chance of edge-case bugs.

### Vite to the rescue!

Astro 6 was a chance to finally do something about this. The new **Vite Environment API** had just been released, which added proper support for non-Node.js environments to Vite. By leveraging this API internally, Astro can now run your web application inside the same runtime you deploy to, with the same JavaScript engine, the same globals, and the same platform APIs available during development.

This refactor enables Astro to:

1. **Abstract away Node.js assumptions** – The dev server is no longer tied to Node-specific APIs.
2. **Run against real runtimes** – Development can execute inside the same runtime as production.
3. **Support more platforms** – Cloudflare Workers today, with a foundation that supports additional runtimes in the future.


This change makes Astro 6 more stable for project on all runtimes, including Node.js. This refactor allowed us to bring Astro's development and production codepaths much closer together. By using the same runtime and APIs in dev as in prod, we've already discovered and fixed numerous subtle bugs that existed only in development or only in production.
...
Astro automatically generates and injects the appropriate CSP headers and nonces for your scripts and styles. See the [CSP configuration reference](https://v6.docs.astro.build/en/reference/configuration-reference/#securitycsp) for all available options.
