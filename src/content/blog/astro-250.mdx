...
- **[Data collections and references](#data-collections-and-references)**: Your `src/content` folder can now includes JSON and YAML files to store data. You can reference this data from your other collections.
...
Content collections are Astro's first-class story to managing and authoring content. Astro 2.5 takes that story even further with **new data formats and collection references.**

First, we've introduced a new `type: 'data'` property to store data formats like JSON and YAML in any collection. This unlocks new forms of content including author profiles, reusable image alt text, translation dictionaries, and more.

Create data collections alongside your existing content:
...
And configure using the new `type: 'data'` property:
...
You may also want to "reference" related entries. A common example is a blog post that references reusable author profiles stored as JSON, or related post URLs stored in the same collection. You can configure these relationships using the new `reference()` function:
...
In 2.0 we added the ability to prerender pages in SSR apps. Now in 2.5 you can do the opposite; in a mostly static site you can allow some routes to _not_ be prerendered.

The new `'hybrid'` output option is what enables this. This new option will be marked as experimental for a short period of time as we stabilize edge cases. You can use it today by enabling the `hybridOutput` flag in the `experimental` section of your config and adding an adapter:
...
Once you've done that your static site can not _opt out_ of prerendering by setting the `prerender` export of a page or endpoint to `false`:
...
Integrations can add these through the `astro:config:setup` hook's `addClientDirective()` API. To enable this API, the user needs to enable `customClientDirectives` in the `experimental` section of their config.
...
See the [LINK TO DOCS](#) to learn how to build these types of integrations.
