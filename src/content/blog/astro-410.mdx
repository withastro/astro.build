...
- **[New a11y audits rules](#new-audits-rules)**
- **[Ability to set `rootMargin` for `client:visible`](#extended-client-visible-directive)**
...
## New a11y audits rules

We've added two new audit rules for the dev toolbar:
...
The `client:visible` client directive now accepts a `rootMargin` option, which allows you to specify a margin around the viewport for the purposes of calculating visibility. This allows a component to be hydrated when it is close to the viewport instead of waiting for it to become actually visible.
...
Cookie encoding / decoding can now be customized through the new `encode` and `decode` functions that can be passed to `Astro.cookies.set` and `Astro.cookies.get` respectively.
