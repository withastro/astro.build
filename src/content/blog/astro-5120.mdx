...
Along with these features comes a way to configure them. The Netlify adapter now accepts a `devFeatures` option in its configuration, allowing you to enable or disable specific features during development. You can set this to `true` or `false` to enable or disable all features, or you can configure them individually. Currently the features you can configure are:

- `images`: Enables the local Netlify Image CDN for Astro images during dev. Default to `true`.
