...
Then, in script tags, you can use the `document.prerendering` property to check if the page is being prerendered:
...
Historically, we've been hesitant to change this behavior due to how many unintended issues changes to the routing system can cause. However, thanks to [Luiz Ferraz](https://github.com/Fryuni) carefully implementing a fix, this is now solved in Astro 4.2! Routes injected using the `injectRoute()` API will now follow the same priority that routes from the filesystem do, following [the routing priority rules outlined in our documentation](https://docs.astro.build/en/core-concepts/routing/#route-priority-order).

As this is a breaking change, we've added a new `experimental` option to the `astro.config.mjs` file to opt-in to the new behavior:
