...
In Astro 5, it is now easier than ever to create a website with both static, cached, performant pages and server-rendered, dynamic pages. Astro's new `output: 'static'` mode, the default, now allows you to render individual routes at runtime on the server just by adding an adapter, no other configuration required.

This is identical to our previous `hybrid` mode: by default, every pages is statically generated until you opt out of prerendering:
