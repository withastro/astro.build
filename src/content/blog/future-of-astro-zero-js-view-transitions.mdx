...
socialImage: "/src/content/blog/_images/future-of-astro-zero-js-view-transitions/og-astro-zerojs-transitions.webp"
lang: "en"
---
...
<BlogSummary>
  This is Part 1 of our series on *The Future of Astro* covering three major new features we have planned for Astro in 2024. This post introduces an update to the [View Transitions API](https://docs.astro.build/en/guides/view-transitions/) that Astro can now leverage for native, app-like page navigation without a single line of JavaScript required.
</BlogSummary>
...
Both Chrome and Edge offer staggered rollout of new versions, so check that your browser is running 126 before running the demo. In chrome, you can see your current version at `chrome://settings/help`.

Open the [astro-zerojs-transitions.vercel.app](https://astro-zerojs-transitions.vercel.app/) demo to experience zero-JavaScript view transitions in your browser. You can explore your browser's Network Panel to verify that every link click triggers a full page load, and zero JavaScript is being sent to the browser to run these animations. If your browser supports it, the demo should look like this:
...
To switch to the native new View Transition API, it’s as simple as adding the `@view-transition` at-rule to your website. Here’s all the CSS you need:
...
If you were already using Astro View Transitions powered by our `<ViewTransitions />` component, then you are now safe to remove that component as well. This will remove the injected backwards-compatible JavaScript router from your page, reducing the overall complexity of your client-side code.
...
If you would still like your view transitions to run in older browsers, you are also welcome to keep using our backwards-compatible `<ViewTransitions />` component. You can migrate whenever you are ready. We have no plans to remove support for the `<ViewTransitions />` component at this time.

## What about transition:persist?

[Persistent element transitions](https://docs.astro.build/en/guides/view-transitions/#maintaining-state) became a popular feature in Astro. The `transition:persist` directive told Astro to persist an element across navigation without interrupting or disrupting it's state. This enabled UI like audio and video players to continue to play across pages, like in <a href="https://twitter.com/charca">Maxi Ferreira's</a> <a href="https://astro-records.pages.dev/">music player demo</a>.
...
We believe this is one of the most significant advancements to come out of Chromium and the browser spec process in recent memory. Following links across websites is a fundamental behavior of the web, and this represents the largest meaningful change in recent memory to how that behavior works.

Developers can take advantage of this feature today or wait as it rolls out to more users. Whenever you're ready, it’s essentially a one-line change of CSS that requires no architectural overhaul to your existing Astro code to enable. To get started, you can learn more about [Astro View Transitions](https://docs.astro.build/en/guides/view-transitions). We will be updating our documentation over the next few weeks to point developers to these [new native browser APIs](https://developer.chrome.com/docs/web-platform/view-transitions).
