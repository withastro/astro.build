...
By default, prerendered pages will still use meta tags, but there is now core support for generating headers for prerendered pages as well. Eventually, this will be implemented in all adapters, but currently only the `@astrojs/netlify` adapter supports it. To enable this, ensure you have updated to the latest version of `@astrojs/netlify`, and add the `experimentalStaticHeaders` option to your adapter configuration:
