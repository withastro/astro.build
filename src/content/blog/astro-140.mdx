...
With `Astro.cookies` you can read and write cookies with a [Map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map)-like API. In the above example we are checking if a cookie exists, and if not setting a default value, then finally using the value in our template.
...
1. CSS you import in your frontmatter, ordered by the order of your import statements (depth-first).
