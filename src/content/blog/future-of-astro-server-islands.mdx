...
In 2021, Astro pioneered a new frontend architecture called [Islands](https://docs.astro.build/en/concepts/islands/). Islands are unique because they allow Astro to automatically strip all unused JavaScript from your page, delivering faster perfomance without forcing you to give up your favorite UI components at dev time (React, Svelte, Vue, etc). 

But frontend performance isn't the only performance problem you'll run into on the web. *How* you deliver a website to the user can be even more impactful that *what* you deliver, and developers are often forced to make a difficult choice: performance, or personalization?
...
You define a server island with a template directive the same way you would a client island, but instead of a `client:*` directive you would use the new `server:defer` directive:
...
However, we believe that server islands make sense for the vast majority of personalized content in content-drive websites. The websites that we see built with Astro often mix dynamic and static content on the page. For these sites that currently can't leverage CDN caching, server islands will likely deliver big performance wins.

## Next Steps

We're in the [design phase](https://github.com/withastro/roadmap/issues/945) of this feature and are thinking about trade-offs for different implementations, but prototyping is already under way.
