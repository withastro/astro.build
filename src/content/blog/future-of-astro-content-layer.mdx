...
const graceHopperProfile = await getEntry('blog', Astro.params.slug);
```
...
We'll be exploring this pluggable ecosystem story in parallel with the new Content Layer API. We are excited to give Astro users something will that unlock entirely new opportunities for composable content without sacrificing the ease of our current built-in Markdown support that you know and love.
