...
- [**Manual routing strategy for internationalization**](#manual-routing-strategy-for-internationalization)
...
Astro 4.6 introduces the ability to move the dev toolbar to different positions around the screen. This can be useful if you have a sticky header that covers the bottom of the screen, or if you just prefer the toolbar to be somewhere else. Thanks to [Ming-jun Lu](https://github.com/mingjunlu) for contributing this feature!
...
Thanks to [Farzard](https://github.com/fshafiee), Astro's Cookie helper for deleting cookies (`Astro.cookies.delete`) now allowing setting all the standard cookie attributes instead of just the `path` and `domain` attributes. Perhaps small change, but definitely a useful one!
