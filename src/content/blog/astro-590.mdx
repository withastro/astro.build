---
title: 'Astro 5.9'
description: 'Astro 5.9 has got your site on lockdown, with experimental support for Content Security Policy'
homepageLink:
  title: 'Astro 5.9'
...
**Astro 5.9 has got your site on lockdown, with experimental support for Content Security Policy**

🛡️ Fortified and ready for takeoff:
...
One of the reasons why users like Astro is because it works everywhere – static hosts, serverless hosts, Node.js, edge runtimes and more, and with
many frontend libraries (React, Vue, Svelte, etc). In this spirit, CSP will work everywhere with any library. How? Let's start by digging-in to the problem.

There are a few solutions that web developers can use to support CSP:
...
The generation of hashes is more complex to implement because it requires knowing the exact content of every script and stylesheet on the page, however it can support more use cases than the `nonce` header. The CSP standard allows hashes to be provided to the browser via the `Response`
...
is the same for both. As you may have guessed, using the `Response` header wouldn't work for Astro, because it would leave out static websites
and SPAs. For this reason, we decided to use the `<meta>` element to provide the CSP to the browser.
...
```
You can now build your project with all the Astro Islands that you want, and they will work out of the box!

If you use the `Content-Security-Policy` header in your website via middleware or other means, you can still use it, and the browser will use the strictest policy of the header and the `<meta>` element.
...
For more details, see the [experimental responsive images documentation](https://docs.astro.build/en/reference/experimental-flags/responsive-images/#responsive-image-properties).
