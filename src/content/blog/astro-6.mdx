...
One important migration note: the Rust compiler is stricter about invalid HTML (e.g. omitted tags) and fixes known issues with tables. In return, you get clearer errors and a stronger foundation for better editor tooling.
...
Content Security Policy is now stable, and we're proud to say that Astro is the first meta-framework to support it for static and dynamic pages, as well as server and serverless environments. The Astro way.
...
Queued rendering leverage a new rendering strategy that is based on a queue. Some early benchmarks have shown improvements up to 2x in rendering speed!

With queued rendering, the components of the tree are first collected in an ordered list, and then they are iterated and rendered. When Astro tries to render a JSX element or an Astro component, it does so with a recursion. The rendering functions are called inside themselves. While recursion works most of the time, sometimes it can create some memory pressure, especially when working with tree-like structures.
