...
Today you must choose 1 caching strategy for all of these types of content, and if the page is a logged-in experience that usually means no caching at all. With Server islands you get the best of both worlds.

<BlogContentImage src={islandDiag} alt="A diagram showing the server island population parts of the page from the server." />

Server islands are used for your most dynamic content; personalized content like the user's avatar, their shopping cart, and product reviews. With these components deferred you're free to more aggressively cache the page itself.
...
[server-islands.com](https://server-islands.com) is consistently faster in repeated runs; it's about 28% faster in the critical First Contentful Paint metric.

With Server islands there is a small waterfall caused by the islands depending on script first running, whereas partial prerendering starts its requests on the server. This means that while Server islands paint earlier, they might finishing later than PPR.

The less cacheable the main content is, the less likely you will want to use either of these techniques. The ability to edge cache the main page content and dynamically load personalized content is the draw here, and we designed Server islands with this in mind. It works well for ecommerce but would probably not be something you want to use for a social network feed.
