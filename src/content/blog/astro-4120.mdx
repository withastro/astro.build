...
Overall we are happy with the trade-offs we've made for Server Islands. Our implementation happens mostly at build-time, where component content is swapped out for a small script, one that can almost fit into a Tweet. This means that Server Islands aren't dependant on any particular server infrastructure and will run on any host.
