...
Astro was born to create content-driven websites, and while it can now be used to build all sorts of dynamic apps too, it is still the very best for sites that are built around lots of content. Whether that is the very best docs sites built with Astro and Starlight for companies such as [Cloudflare](https://developers.cloudflare.com/) and [Stackblitz](https://tutorialkit.dev/) or beautiful marketing sites for brands such as [Porsche](https://www.porsche.com/usa/) and [Netlify](https://www.netlify.com), millions of users every day are already appreciating the fast and accessible sites that you can build with Astro, while thousands of engineers love the best developer experience in the industry.
...
The Content Layer API is a big step forward for Astro, but we're just getting started. Current the data store is just a key value store, with limited filtering. This is fast but not memory efficient, and querying is not very flexibile. Our goal is to introduce a backend based on Astro DB in a future version, which should help it scale to hundreds of thousands of pages. It will also allow us to support more complex queries, and perhaps even real-time updates.

In the meantime, we're love to hear your feedback on the Content Layer API. Try migrating your existing sites to use it ([it's easy, we promise!](https://5-0-0-beta.docs.astro.build/en/guides/upgrade-to/v5/#legacy-v20-content-collections-api)) and let us know how it goes. [Try building a loader](https://5-0-0-beta.docs.astro.build/en/reference/loader-reference/) for your favorite API, or even [build support into an integration](https://5-0-0-beta.docs.astro.build/en/reference/integrations-reference/#refreshcontent-option). We're excited to see what you build with it!