...
Although immutable in production, the data store can be updated on demand by users with the `s+enter` hotkey when running `astro dev` or by integrations. This could be used for things such as registering a development refresh endpoint or opening a socket to a CMS to listen for updates.
