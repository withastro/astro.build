...
The `@astrojs/tailwind` integration is now deprecated, but will continue to work for older versions of Tailwind. If you want to upgrade, please uninstall the integration and either use the updated `astro add tailwind` command or follow the [Tailwind documentation for manual installation](https://tailwindcss.com/docs/installation/framework-guides/astro).
...
Currently Astro supports YAML frontmatter in Markdown and MDX files. With this release, Astro now also supports TOML frontmatter in both. This is useful for adding existing content files with TOML frontmatter to your project from another framework such as Hugo, or you may just prefer TOML over YAML!
