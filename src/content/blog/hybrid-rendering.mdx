...
We'll simplify things a bit for clarity, but we first need to understand that Astro's build process happens in multiple stages. Initially, Astro uses [Vite](https://vitejs.dev) to generate a server-side JavaScript bundle. While this bundle is being generated, we perform static analysis on your source code to generate a client manifest. The output of this bundle includes:

-   **Server-side JavaScript** used to render HTML
-   A **client manifest** which collects all components needed for client-side interactivity
