...
We'll simplify things a bit for clarity, but we first need to understand that Astro's build process happens in multiple stages. First, Astro uses [Vite](https://vitejs.dev) to generate a server-side JavaScript bundle. While this bundle is being generated, we perform static analysis on your source code to generate a **client manifest**. The output of this bundle includes:

-   **Server-side JavaScript** used to render HTML
-   A **client manifest** which marks any components needed for client-side interactivity
...
During the initial bundling process, we've added an additional static analysis step that determines which pages should be prerendered.
...
Much like the original `server` process, the server chunk is passed to an [`adapter`](https://docs.astro.build/en/reference/configuration-reference/#adapter) for further processing. It will ultimately be deployed as a Serverless or Edge function.
...
To do this efficiently, we rely on the wonderful [`es-module-lexer`](https://github.com/guybedford/es-module-lexer) library, which is used by Node itself.
...
Hybrid Rendering unlocks a whole new set of possibile use cases, but we wanted to highlight a few that we're particularly excited about.
...
Hybrid Rendering is available today in Astro 2.0! You can head to our [server-side rendering guide](https://docs.astro.build/en/guides/server-side-rendering/) to get started.

Astro's official adapters have also been updated, so be sure to install the latest version of your [adapter](https://docs.astro.build/en/guides/server-side-rendering/#adding-an-adapter).

We also welcome your feedback on Hybrid Rendering! We collected some wonderful feedback during the [Prerender API RFC](https://github.com/withastro/roadmap/blob/main/proposals/0029-prerender-api.md) and invite you all to propose future ideas in our new [public roadmap](https://github.com/withastro/roadmap).
