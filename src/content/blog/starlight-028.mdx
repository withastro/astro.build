...
- [Sparkling new component docs](#sparkling-new-component-docs)
...
Use the new `Astro.locals.t()` method from any Astro component to use Starlight’s built-in UI labels or your own custom strings, including support for interpolation and pluralization:
...
This update makes Starlight an unrivalled choice for out-of-the-box internationalization features.
Even if starting with a single language today, build with the confidence that you’ll be ready to translate your documentation when the time comes.
...
## Sparkling new component docs

We have overhauled the documentation for Starlight’s built-in components.
Each component now has a dedicated page including common usage examples and a full prop reference.

Start with the new [“Using components” guide](https://starlight.astro.build/components/using-components/) to explore the available components.

## A Markdoc preset for the adventurous

Starlight users interested in Markdoc can now use the new `@astrojs/starlight-markdoc` package to quickly configure all of Starlight’s built-in components for use in Markdoc files.
...
import FooterLinks from './_components/FooterLinks.astro';

<FooterLinks slugs={['starlight-turns-one', 'wp-engine']} />
