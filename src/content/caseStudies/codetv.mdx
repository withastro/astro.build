...
Jason chose Mux to provide adaptive bit-rate streaming of subscriber-only video content.
...
Astro's extensibility meant that adding the necessary integrations to CodeTV to build out the streaming platform went smoothly: Clerk for authentication, Stripe for payment processing, Mux for streaming video, Sanity for content management, and Inngest for background tasks.
...
But when Jason does have to dive into the code because new features are required, he's happy to be there! Astro code makes sense, is easy to jump back into, and there are no struggles optimizing API calls or database latency.
