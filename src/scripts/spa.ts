...
async function onClick(event: MouseEvent) {
    ...
    return navigateTo(new URL(href, window.location.toString()));
}

async function get(url: URL) {
    const html = await fetch(url.toString()).then(res => {
...
async function navigateTo(url: URL, replace = false) {
    ...
    swap(document.querySelector('[astro-icon-spritesheet]'), newDoc.querySelector('[astro-icon-spritesheet]'));
