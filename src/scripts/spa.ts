...
if (window.matchMedia('(hover: hover)').matches) {
	...
	const set = (url: URL) => {
		...
		activeTimeout = setTimeout(() => {
			fetch(url.toString())
				.then(res => res.text())
				.then(text => cache.set(url.toString(), text))
		}, 600)
	}
	const clear = () => {
		if (activeTimeout) {
...
async function navigate(url: URL, isBack: boolean = false) {
	...
	await document
		.querySelector('#root')
		.animate({ opacity: 0 }, { duration: 120, easing: 'ease-out' }).finished
	...
	await document
		.querySelector('#root')
		.animate({ opacity: 1 }, { duration: 80, easing: 'ease-in' }).finished
