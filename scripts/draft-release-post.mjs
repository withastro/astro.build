...
async function main() {
  let changesets = await githubGet({ url: 'https://api.github.com/repos/withastro/astro/contents/.changeset', githubToken })
  ...
  let version = await fetch('https://raw.githubusercontent.com/withastro/astro/main/packages/astro/package.json').then(res => res.json()).then(json => json.version)
  let oldVersion = version
  if (main.find(item => item.increment === 'major')) version = version.split('.').map((v, i) => i === 0 ? parseInt(v) + 1 : 0).join('.')
  else if (main.find(item => item.increment === 'minor')) version = version.split('.').map((v, i) => i === 1 ? parseInt(v) + 1 : 0).join('.')
